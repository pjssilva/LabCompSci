...
A ideia é dar um valor para cada píxel que represente a sua importância para o conteúdo na imagem. A partir desses pesos procurase uma faixa vertical ou horizontal de píxeis conectados de peso total mínimo e esses pixeis são retirados da imagem. Isso permite mudar o tamanho da imagem sem mudar os objetos importantes, livrando-se de "espaço morto ou inútil".

Notem que essa busca de _faixas de píxeis conectados de custo mínimo_ está intimamente ligada com a nossa solução do problema da descida. De fato, vamos usar o mesmo algoritmo de otimização dinâmica para resolver o problema, com alguns ajuste para facilitar o trabalho final.

Mas para atingir esse objetivo, precisamos primeiro definir um nídel de importância para cada pixel.
"""
...
Quem quiser pode assistir o vídeo do Grant Sanderson (3Blue1Brown) explicando as ideais do _seam carving_ usado o código original desse caderno na versão do MIT desse curso oferecida no outono de 2020. Mas eu, ousadamente, irei fazer a minha própria apresentação
"""
...
Incialmente, precisamos definir uma noção de **importância** para cada píxel. Uma custura então será uma faixa de píxeis conectados e o nosso desejo é usar as costuras de menor importância total. 

Nossa opção aqui será usar definir a importância como "o quanto que cada píxel está dentro de uma borda da imagem". Ou seja, consdieramos que são as bordas, ou contornos, que definem a informação mais importante do ponto de vista visual. 
...
begin
    image_urls = [
        ...
        "https://upload.wikimedia.org/wikipedia/commons/thumb/1/17/Gustave_Caillebotte_-_Paris_Street%3B_Rainy_Day_-_Google_Art_Project.jpg/1014px-Gustave_Caillebotte_-_Paris_Street%3B_Rainy_Day_-_Google_Art_Project.jpg" =>
            "Rua de Paris em um dia de chuva",
...
md"""
Para detectar bordas, precisamos converter a imagem para preto e branco. Vamos fazer isso baseados no brilho de cada píxel. Isso pode ser feito com uma média simples dos valores nos canais RGB ou usando uma média poderada que [aproxima melhor a nossa percepção](https://www.tutorialspoint.com/dip/grayscale_to_rgb_conversion.htm).
"""
...
\end{align}
```
Aqui a $\star$ denota a convolução e $A$ é o array correpondente à imagem. 

Assim, as matrizes  $G_x$ e $G_y$ teram informação das **derivadas** (discretizadas) da informação da imagem nas direções $x$ e $y$ repectivamente.

Por fim, combinamos essas duas informações calculando a magnitude do **gradiente** (discretizado), no sentido do que apredemos em cálculo 2:
...
md"""
Veja represetações gráficas dos núcleos usados em cada direção
"""
...
Abaixo vemos detalhes com mais clareza.
"""

# ╔═╡ 88b8b928-b3bd-419f-8daa-f814a027d172
md"""Por fim, combinamos os dois nucleos direcionais no núcleo de Sobel, obtendo um detector de bordas bem interessante.
"""
...
Como já cometamos, a ideia por trás do _seam carving_ é encontrar um caminho vertical, ou horizontal, de píxeis conectados (pelo menos pelos cantos) que minimize uma medida de "borda". Na nossa implementação vamos usar a função `edgeness` acima. Assim, o caminho tenta minizar as bordas que ele cruzam, sugerindo eliminar regiões com menor "variação" ou "informação". 
...
Não por acaso, no caso de passos verticais, esse é exatamente o problema da descida descrito na última aula. Lembre que ele foi resolvido de forma eficiente por um algoritmo de otimização dinâmica. Colocamos aqui uma implementação alternativa do método de otimização dinâmica que devolve duas matrizes: 

* A matriz final dos custos de energia mínima para descer a partir da cada ponto.
* Uma matriz com os vetores de direção da discida codificados como -1 (esquerda), 0 (vertical) e +1 (direita).
...
md"""
Vamos recortar a costura vertival mínima, efetivamente diminuindo a largura da imagem de um píxel, em uma especíe de algoritmo de compressão.
"""
...
md"""
Escolha o número de constura para remover.
