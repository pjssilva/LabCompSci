...
``f(y, a)= \left\{ \begin{array}{l} -1 + a - y, \ \ {\rm se} \ y < 0  \ \ 
...
md"""
O gráfico da superfície `z = f(y,a)` consiste de dois semiplanos paralelos. Na figura abaixo, à esquerda, apresentamos gráfico da função para valores de `a` fixos (uma seção da superfície). À direita apresentamos as raízes, os pontos que fazem `z = 0` (a seção com plano z = 0).
"""
...
begin
    ...
    if a < 1
        ...
        annotate!(
            -2.2 + a,
            -4,
            text(round(-1 + a, digits=1), 11, :red),
        )
        scatter!([-1 + a], [0], m=:circle, ms=10, color=:red)
    end
    if a > -1

        annotate!(1 + a, -4, text(round(1 + a, digits=1), 11, :blue))
    ...
    begin
        ...
        if a > -1
            annotate!(
                -4,
                1.3 + a,
                text(round(1 + a, digits=1), 11, :blue),
            )
            hline!([1 + a], c=:gray, ls=:dash)
...
function plotit(y₀, a)
    ...
    plot!(xs, ys, arrow=true, alpha=0.3, c=:lightgray)
    ylabel!("y")
...
md"""
### Histerese: aumentado e _depois_ diminuindo  ``a``

Vamos aumentar a em passos de 0.25 indo de -4 até 4 e depois decrescer de 4 a -4. Toda vez que aumentarmos a vamos permitir que passem 10 passos de tempo, para que o novo equilíbrio, associado àquele a, seja alcançado. 

Vemos aí o fenômeno de histerese. Para -1 < a < 1 o sistema pode se acomodar em torno do equilíbrio negativo ou positivo dependendo de como chegou lá.
"""
...
let
    ...
    for i = 1:32
        ...
        if -1 ≤ a ≤ 1
            ...
            p = plot!(sol, lw=2, xlims=(0, t), legend=false)
        end
...
md"""
Como já comentamos, essa dependência com do estado do sistema com a sua história (a história de seus parâmetros), que vemos acima é o chamamos de *histerese* (do grego (ὑστέρησις) para ficando para trás).
"""
...
A dinâmica dessa **Equação diferencial ordinária (EDO)** é bastante simples, já que é _linear_. Nós podemos reescrevê-la na forma:

$\dot{T} = f(T(t)),$ 

em que $f(x) = \alpha x + \beta$ é um função _linear_ de $x$. Uma equação linear possui uma única solução estável $\dot{T} = 0$, que na aula anterior era a temperatura terrestre do período pré-industrial $T_{0} = 14$°C. 

Nessa aula vamos ver que uma pequena modificação na equação torna um termo no nosso modelo climático simples não linear. Isso muda completamente a dinâmica, permitindo a existência de dois equilíbrios: a "Terra bola de neve" e o clima relativamente quente do período pré-industrial que permitiu que os seres humanos florescessem.
"""
...
Evidências geológicas mostram que na período Neoproterozoico (de 550 a 1000 milhões de anos atrás) é caracterizados por dois eventos de glaciação global, durante os quais a superfície da Terra estava _completamente_ coberta de gelo, do equador aos polos (veja  uma revisão sobre esse assunto [Pierrehumbert et al. 2011](https://www.annualreviews.org/doi/full/10.1146/annurev-earth-040809-152447)).
"""
...
Anteriormente usamos um valor **constante**  $α =$ $(Model.hist.α) para o albedo da Terra. Isso é razoável para pequenas variações climáticas relativas ao presente, como quando comparamos o clima atual e o pré-industrial. Porém, quando as variações são grandes, essa aproximação não é muito confiável.

Se por um lado os oceanos são escuros e absorventes, $α_{ocean} \approx 0.3$,
o gelo e a neve são claros e refletivos: $\alpha_{ice,\,snow} \approx 0.5$ a $0.9$. Desse modo, se grande parte da superfície oceânica congelar, é esperado que o albedo da Terra aumente de forma dramática, refletindo mais radiação solar para o espaço. Isso, por sua vez, causa mais resfriamento, aumentando o congelamento dos oceanos. Esse é um efeito de *retorno não-linear positivo* conhecido como **retorno neve-albedo**. Veja a ilustração abaixo.
"""
...
md"""
Nós podemos representar o retorno neve-albedo grosseiramente em nosso modelo de balanço de energia permitindo que o albedo dependa da temperatura:

$\alpha(T) = \begin{cases}
\alpha_{i}, & \mbox{if }\;\; T \leq -10\text{°C} &\text{(completamenteo congelado)}\\
\alpha_{i} + (\alpha_{0}-\alpha_{i})\frac{T + 10}{20}, & \mbox{if }\;\; -10\text{°C} \leq T \leq 10\text{°C} &\text{(parcialmente congelado)}\\
\alpha_{0}, &\mbox{if }\;\; T \geq 10\text{°C} &\text{(sem gelo)}
\end{cases}$
"""
...
function Model.timestep!(ebm)
    ...
    append!(ebm.T, ebm.T[end] + ebm.Δt * Model.tendency(ebm)) # Update temperature
    append!(ebm.t, ebm.t[end] + ebm.Δt) # Update time
end
...
A civilização humana floresceu nos últimos milhares de anos em parte porque o clima global tem se mostrado muito estável e acolhedor. A combinação dos gases de efeito estufa e da radiação enviada pelo Sol gerou um ambiente com temperatura entre os pontos de congelamento e ebulição da água na maior parte da superfície terrestre, permitindo a proliferação de múltiplos ecossistemas. 

O sistema do clima, porém, possui vários efeitos não-lineares, como e **efeito da neve-albedo**, que sugerem o quão frágil é esse equilíbrio e o quão único era o clima estável pré-industrial.

Nós aprendemos nos últimos 20 anos que, em resposta a flutuações na temperatura, **retroalimentações líquidas negativas** agem para restaurar a temperatura do planeta ao único equilíbrio no qual a energia solar absorvida é balanceada pela perda de energia térmica para o espaço. Vamos agora explorar como **retornos não-lineares positivos** podem temporariamente resultar em um **retorno líquido positivo** modificando o espaço de estado do clima do planeta.
"""
...
md"Nós conseguimos ter uma ideia do comportamento geral desenhando várias curvas no mesmo gráfico."
...
md"Vemos que para  T₀ ⪆  $(round(T_un, digits=2)) °C, todas as curvas parecem convergir parar o *equilíbrio* (ou *ponto fixo*) T = 14°C que já conhecemos. Trajetórias que começam acima desse valor resfriam com o tempo. Já trajetórias que começam abaixo esquentam. Já as trajetórias em que T₀ ⪅ $(round(T_un, digits=2)) °C, convergem para um equilíbrio muito mais frio, de cerca de -38.5°C. Essa é o equilíbrio da **Terra bola de neve**. Esses dois estados são chamados de _estados de equilíbrio_ porque se o temperatura é temporariamente desviada dessas situações ela terá a tendência a retornar ao equilíbrio original. 

Mas o que ocorre para T₀ ≈ $(round(T_un, digits=2)) °C? Para alguma temperatura próxima existe um outro equilíbrio que é _instável_. Se o sistema começa exatamente nele ficará nele para sempre. Mas mesmo pequenos desvios levam a busca de um dos outros dois equilíbrios que são _estáveis_. Eles são _atratores_.
"
...
Lembre-se que, com uma concentração de CO₂ fixa, nosso sistema de balaço de energia pode ser expresso na forma:
...
Em particular, se desenharmos o lado direito da EDO dos dois termos em função da variável de estado $T$, podemos desenhar um diagrama de estabilidade do sistema que nos dirá se o planeta ira aquecer ($C \frac{d\,T}{d\,t} > 0$) ou resfriar ($C \frac{d\,T}{d\,t} < 0$).
"""
...
##### 3.2) O aumento de luminosidade do Sol derreteu a bola de neve?
...
if extend_S
    md"""
       ...
       O que essa nossa simulação nos ensina é que não devemos confiar demais na estabilidade de nosso clima e que permitir que o clima atual saia fora de sua faixa histórica pode, hipoteticamente, disparar uma dessas transições climáticas abruptas que seriam desastrosas.
       """
end
...
A teoria dominante é que uma emissão lenta e gradual de CO₂ por vulcões levou um efeito estufa forte o suficiente para compensar o efeito de resfriamento ocasionado pelo alto albedo da superfície terrestre completamente coberta de gelo e neve permitindo ultrapassar o ponto de derretimento de $-10$°C.
"""
...
md"""
Na **lista de exercícios**, você irá estender o modelo acima para incluir o efeito do CO₂ e determinar quanto gás carbônico seria necessário adicionar à bola de neve para que ela derretesse.
"""

# ╔═╡ 66c35971-30fb-4743-b65e-d2e527e15be7
md"""### 4) Em direção a um modelo climático realista

Neste modelo simplificado, o clima preindustrial de $T=14$°C é tão quente que não há gelo sobre o planeta. De fato, como ele não diferencia a superfície terrestre há apenas dois equilíbrios estáveis, um no qual _não haveria gelo em nenhum lugar_ e outro em que _haveria gelo por toda parte_.

Mas como entender então as capas polares terem resistido por um longo período quente pré-industrial? 
"""
...
O vídeo abaixo mostra que a simulação Aquaplanet possui um terceiro estado de equilíbrio com um oceano _majoritariamente líquido mas com capas polares congeladas_.
"""
...
md"""
Para insolações $S$ entre $(Smin) W/m² e $(Smax_limited) W/m², as temperaturas sempre permanecem abaixo de -10°C e o planeta permanece completamente congelado. O que ocorre se estendermos os limites superiores e inferiores de insolação para simular um aumento do brilho solar a ponto de começar a derreter o gelo?
"""
...
begin
    final_temp = @sprintf("%.1f°C", ebm_interact.T[end])
    p_interact = plot(ebm_interact.t, ebm_interact.T, label=final_temp, lw=3)
...
manifest_format = "2.0"
project_hash = "41af4060f696235519ebe6043fe4a799daaf5d72"
