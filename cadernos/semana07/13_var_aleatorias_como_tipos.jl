...
md"Tradução livre de [random\_variables\_as\_types.jl](https://github.com/mitmath/18S191/blob/Spring21/notebooks/week7/random_variables_as_types.jl)"
...
$\mathbb{P}(X = x)$. Isso no caso discreto. Já para variáveis aleatórias contínuas não é possível definir a probabilidade de valerem um valor específico. O que medimos é a probabilidade do valor amostrado pertencer a "intervalos".

A forma como essas probabilidades se distribuem em todos os resultados possíveis é chamada de **distribuição de probabilidade** da variável aleatória.
...
md"""
Vamos usar como exemplo inicial um tipo muito importante de distribuição de probabilidade: a **distribuição gaussiana ou distribuição normal** com média $\mu$ e variância $\sigma^2$ (ou desvio padrão $\sigma$).

Nós podemos amostrar de uma distribuição gaussiana com média $0$ e variância $1$ usando a função `randn` (que quer dizer "random normal"). Podemos, então, deslocar e mudar a escala para obter uma distribuição gaussiana qualquer:
"""
...
md"""
Vamos agora tentar **somar** duas distribuições gaussianas. Do ponto de vista empírico (experimental) a soma de duas variáveis aleatórias é fácil de calcular. Basta amostrar as duas e somar os resultados dos experimentos:
"""
...
O que uma variável aleatória teórica deveria ser capaz de fazer? O que deveria ser possível de se fazer a partir de uma amostra (ou com amostragem) ?
...
- O produto teórico de duas variáveis aleatórias.

- Distribuição de probabilidade.

### Amostras

- Obtidas a partir da **amostragem** de uma variável aleatória com distribuição determinada.
- Média da amostra.
...
Algumas linguagens permitem falar naturalmente de várias funções associadas a uma distribuição/variável aleatória, mas não é possível falar da distribuição/variável aleatória em si.

Vejamos o exemplo de R. Em R há padrões que definem regras, convenções, para representar densidades (`d`), por exemplo, seguidos de um nome como `norm` para definir qual a distribuição que possui aquela densidade. Mas não há um nome para falar da distribuição em si. As funções típicas são:
...
Não deixa de ser curioso que todos esses objetos falem de características de uma distribuição (ou variável aleatória) que você pode encontrar em qualquer curso de probabilidade, mas ao mesmo tempo não é possível falar da distribuição em si.
"""

# ╔═╡ f307e3b8-8ff0-11eb-137e-4f9a03bb4d78
md"""
Nós vamos adotar outro caminho, vamos buscar uma forma de se referir a uma  distribuição / "respectiva variável aleatória" específica. 

Obs: **A partir desse ponto vou adotar a postura dos autores que usa o termo "variável aleatória" quase como sinônimo da respectiva distribuição, permitindo amostrar dela múltiplas vezes. Isso conflita com a convenção, por exemplo, do pacote `Distributions.jl` que usa o nome "distribuição" para esses objetos. Por outro lado, isso é uma vantagem, já que permitem que o nosso código conviva com o `Distribution.jl` sem que ocorram conflitos de nomes.**

Nós queremos definir tipos que recebem os parâmetros da distribuição de probabilidade de uma variável aleatória antes de definir exatamente como amostrá-la. Esse é um bom exemplo em que alguma **abstração** com antecedência traz ganhos interessantes.
...
Podemos pensar que um tipo abstrato representa uma coleção de tipos que compartilham algumas propriedades. No nosso caso, queremos criar um tipo abstrato que represente uma distribuição qualquer e, além disso, subtipos abstratos que se especializem para representar distribuições _contínuas_ e _discretas_.

Ao fazer isso poderemos especificar depois tipos _concretos_ (ou seja particulares, especializados, e que podem de fato armazenar informação) de variáveis aleatórias discretas ou contínuas.
...
begin
    ...
    Gaussian() = Gaussian(0.0, 1.0)  # normalized Gaussian with mean 0 and variance 1
end
...
Outro detalhe interessante é que criamos um construtor alternativo que não recebe parâmetros e cria uma variável gaussiana com média 0 e variância 1, que é a mais usual.
"""
...
md"""
Já o produto de duas gaussianas não é uma gaussiana. Vamos lidar com essa situação depois.
"""
...
md"""
Nós também precisamos definir como amostrar de uma distribuição gaussiana. Para isso, vamos estender a função `rand` com um novo método.
"""
...
md"""
Vamos definir a soma de duas variáveis aleatória para *qualquer caso*:
"""
...
md"""
E o que ocorre se somarmos mais de duas variáveis aleatórias?
"""
...
md"""
Agora que definimos o operador `+`, A definição genérica de Julia para `sum` é capaz de lidar com a soma de muitas variáveis aleatórias.
"""
...
md"""
Observe que não foi necessário transformar a expressão acima primeiro em uma vetor com uma compreensão de vetores. Isso é possível porque sum aceita também  **iteradores** ou **expressões geradoras**:
"""
...
md"""
Veja que interessante! Tudo funciona naturalmente!
...
manifest_format = "2.0"
project_hash = "92f187f6fdfff1aa04078de3c5a58132c62b63f8"
...
git-tree-sha1 = "d8c371a269abf3129e81a4adbab956b82005103d"
