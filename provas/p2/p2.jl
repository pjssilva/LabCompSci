...
md"Aqui está uma simulação de pandemia."
...
# Copie aqui o código de interact! e adapte como descrito acima.

# ╔═╡ 39b97f4d-a3bd-4540-85b2-ff01b77e4f60
md"Abaixo você vê uma simulação de sua nova pandemia."

# ╔═╡ 953c2875-c0fa-4dd6-8188-0f635c70daf0
@bind run_simulation2 Button("Run simulation again!")

# ╔═╡ 2f43cc6b-b1cd-4f14-ba7a-157cd10cbeb3
md"""
# 2 Passeios 1D

Na lista seguinte vimos que podemos também introduzir no modelo movimentos nos agentes de maneira a simular encontros que permitem a transmissão entre seres humanos. Vamos fazer isso agora em um ambiente unidimensional.

Precisamos começar generalizando a estrutura `Agent` apresentada abaixo. """
...
# Copie o código de interact do topo da questão 1 e faça suas adaptações

# ╔═╡ 6188a15d-d6b5-4140-abb0-0661703f530d
md"Se você fizer tudo direito deve ver curvas típicas de infecções SIR abaixo a maioria da vezes. Você pode forçar uma nova simulação clicando o botão `Run simulation again`." 

# ╔═╡ ca1319bb-788d-43ee-9eb6-170737234361
@bind run_simulation3 Button("Run simulation again!")
...
Na lista fizemos simulações considerando a função `CO2` constante, aqui vamos permitir ela variar.

Obs: Você não precisa olhar os detalhes do módulo `Model` para resolver esse exercício. Por isso a célula abaixo está colapsada. Mas, se sentir vontade, clique no ícone do "olho riscado" ao lado para célula para revelar o código.
"""

# ╔═╡ acbb47cd-b914-4f3b-9e80-b0e1412a8b8d
module Model
...
# albedo function, or planetary reflectivity [unitless] taking into account the possible freezeing of the planet
function α_function(T; α0=α, αi=0.5, ΔT=10.0)
    if T < -ΔT
        return αi
    elseif -ΔT <= T < ΔT
...
function timestep!(ebm)
	ebm.α = α_function(ebm.T[end]) # Added this line
...
# ╔═╡ 151554d3-7ad0-4f57-b4cb-9714c6e11a6b
tempos = 0:5:1_000
...
# Coloque sua implementação de CO2 aqui, substituindo a implementação abaixo
CO2 = t -> 350

# ╔═╡ f1219e33-5885-47a8-85aa-afe70c97db63
# O Gráfico acima mostra a função que você criou acima para ajudá-lo a ver se está correta.
plot(tempos, CO2.(tempos), lw=2)
...
md"Abaixo irá surgir um gráfico que mostra a evolução de temperaturas obtida e pode ajudar você a saber se sua resposta faz sentido."
...
md"O gráfico a seguir mostra a evolução de temperaturas obtida e pode ajudar você a saber se sua resposta faz sentido."
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection, AgentType::Type)
	...
	for t = 1:T
		...
		S_counts[t], I_counts[t], R_counts[t] = get_counts(agents)
	end
...
let
	...
	N = 100
	T = 1000
	sim = simulation(N, T, InfectionRecovery(0.02, 0.002), Agent)
	T = length(sim.S)
...
# ╔═╡ 9b379c3d-4f3b-49be-9648-9662d412d284
let
	run_simulation3
...
# ╔═╡ a359b28f-dca7-4a79-85fb-8c4e9572c64e
ebm_padrao = Model.EBM(10, 0, 5, t -> 250)

# ╔═╡ b04c2638-421a-4c35-8d93-44eb70f609a2
# Defina ebm_bola_de_neve e faça ela evoluir, substituindo o código abaixo.
ebm_bola_neve = ebm_padrao

# ╔═╡ c062d99e-fbf2-452d-a186-e16f3122c629
plot(ebm_bola_neve.t, ebm_bola_neve.T, lw = 2, label = "", title = "Temperatura partindo de bola de neve")

# ╔═╡ 0145d9b2-ec8c-46c6-abba-1fba321a62bc
# Defina ebm_oceano e faça ela evoluir aqui, substituindo o código abaixo.
ebm_oceano = ebm_padrao

# ╔═╡ cfd49909-46f0-4808-8b32-8ac8a184cd5c
plot(ebm_oceano.t, ebm_oceano.T, lw = 2, label = "", title = "Temperatura partindo de Terra líquida")
...
manifest_format = "2.0"
project_hash = "23b3f41d1251cc64213b373e9f12eb7923824742"
