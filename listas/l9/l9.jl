...
> Como o operador de soma é apenas uma função, podemos adicionar métodos a ela. Essa funcionalidade é bastante útil em linguagens gerais, como Julia ou Python, já que permite que você use uma sintaxe familiar (`a + b * c`) em objetos que não são, necessariamente, números!
> 
...
👉 Escreva um função `trajectory` que calcula a trajetória de um `Coordinate` que representa um andarilho `w` (de _wanderer_, o inglês para andarilho) ao realizar `n` passos aleatórios. Isto é, você deve devolver a sequência de posições que o andarilho percorre.
...
> Cada extremo da caixa funciona como uma parede que não pode ser ultrapassada. Se um andarilho tentar pular além da parede, ele termina na posição dentro dos limites que está mais próxima do seu objetivo.
...
md"""
## **Exercício 2:** _Agentes andarilhos_
...
👉 Escreva uma função `initialize` que recebe parâmetros $N$ e $L$. $N$ é o número de agentes e $2L$ representa os comprimentos da caixa quadrada, centrada na origem, onde os agente "vivem".
...
1. Nem sempre a iteração entre dois agentes na mesma célula é uma iteração perigosa. Por exemplo dois agentes podem só se encontrar de passagem, em movimento. Para modelar isso podemos adicionar um parâmetro $p_I$ que represente que a probabilidade que um encontro gere uma iteração que resultem chance efetiva de transmissão. Nesse caso a função `interact` iria sortear um valor aleatório a mais para decidir se haveria ou não a chance de transmissão com a probabilidade original. É claro que é possível juntar esses dois valores aleatórios em um só, mas conceitualmente é interessante mantê-los separados.
