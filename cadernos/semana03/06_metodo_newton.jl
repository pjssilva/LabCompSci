...
md"""
# Resolvendo equações não-lineares: o método de Newton
"""
...
Se todas as equações forem *lineares*, podemos usar técnicas de álgebra linear computacional, como fatoração LU ou outra variantes mais adequadas para a matriz associada ao sistema. Essas variantes tentam aproveitar alguma estrutura específica da matriz: como simetria, o fato de ser definida positiva ou ainda esparsidade. Hoje em dia, é comum a resolução de sistemas com *milhões de equações e variáveis* mesmo em computadores de mesa.

Já no caso de equações *não*-lineares a situação é um pouco menos clara. Isso ocorre porque esses problema são essencialmente mais difíceis. De fato, sabemos que eles não são resolvíveis em tempo finito, exigindo o emprego de métodos iterativos que aproximam cada vez melhor uma solução.

O rei de todos os métodos para a solução de sistemas não lineares está baseado em uma ideia muito simples: usar a aproximação do sistema obtida pelo sistema linear definido a partir das suas derivadas. Esse é o método de Newton. Vamos entender um pouco como ele funciona enquanto apresentamos outras funcionalidades e pacotes de Julia.

Uma das dificuldades históricas no uso do método de Newton é a necessidade do cálculo de derivadas, se possível exatas, das funções envolvidas. Como vimos isso é resolvido em Julia de forma eficiente e elegante por pacotes de diferenciação automática como o `ForwardDiff.jl`. Essa já é uma grande vantagem, que temos como ponto de partida em relação a outras linguagens em que o uso de tais pacotes é menos óbvio.
...
Vamos considerar que queremos resolver uma equação na reta real da forma $f(x) = g(x)$. Um primeiro passo, tradicional, é isolar o 0 no lado direito e resolver $h(x) := f(x) - g(x) = 0$. Um ponto $x$ em que $h(x) = 0$ é chamado de *zero de* $h$ ou uma *raiz de* $h$.

O método de Newton busca resolver esse problema aproximando a função $h$ por uma reta baseada em sua derivada (o polinômio de Taylor de ordem 1). Se essa reta é não horizontal, ela irá cruzar o eixo $x$ fornecendo uma nova aproximação de raiz que será então refinada iterativamente.
...
md"""
Nós podemos usar um pacote de computação simbólica em Julia, `Symbolics.jl`, para nos ajudar a entender o que está ocorrendo. Pelo menos no caso de polinômios.
...
md"""
Quando $\eta$ é pequeno $\eta^2$ é *muito* menor, então pode ser ignorado. O que
...
md"""
Essa é mais forma de ver que a derivada captura a parte *linear* da função. De fato essa é ideia por trás de `ForwardDiff.jl`, ou qualquer pacote de diferenciação automática progressiva. Eles usam essa informação para propagar a parte linear ao longo do cômputo da função.
"""
...
md"""
Podemos escrever algebricamente a ideia geométrica de "seguir a raiz da reta tangente". Isso permite implementar o método no computador (pense sobre isso). Além disso, se estende "naturalmente" para dimensões maiores.
"""

# ╔═╡ f153b4b8-7ba0-11eb-37ec-4f1a3dbe20e8
md"""
Considere que temos uma estimativa $x_0$ da raiz e queremos encontrar estimativa melhor $x_1$ (com um pouco de sorte). Podemos escrever $x_1 = x_0 + \delta$, como forma de enfatizar que ele é calculado a partir de $x_0$.

Idealmente $x_1$ seria uma raiz, ou seja
"""
...
md"""
Vamos ver o que ocorre quando perturbamos os argumento de uma transformação $T: \mathbb{R}^2 \to \mathbb{R}^2$ por valores pequenos $\delta$ na direção $x$ e $\epsilon$ em $y$ nas proximidades de um ponto $(a, b)$:
"""
...
# The Jacobian matrix, the derivative of T
jacobian(T(α), [a, b])
...
em que $J := DT_{x_0}$ é a matriz jacobiana $T$ em $x_0$. Ou seja, a matriz cujas linhas são formadas pelos gradientes de cada uma das componentes da transformação $T$. Como sabemos, a expressão acima fornece uma aproximação linear de $T$ de alta qualidade próximo a $x_0$.
"""
...
md"""
A grande vantagem da ideia acima é que substituímos um problema que não sabemos resolver - encontrar soluções de sistemas não-lineares - por uma *sequência* de problemas que sabemos resolver muito bem: a solução de sistemas lineares.
"""
...
begin
    function inverse_alt(f, y, u0=[0, 0])
        prob = NonlinearProblem((u, p) -> f(u) - y, u0) # Ignoring p
        solution = solve(prob, NewtonRaphson(), reltol=1e-4)
        return solution.u
    end
