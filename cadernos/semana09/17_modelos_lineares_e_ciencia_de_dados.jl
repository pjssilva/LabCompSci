...
md"""
## Julia: Data Frames
Uma das maneiras de se pensar em um Data Frame é basicamente uma tabela, com rótulos para as colunas.
"""
...
Lembre-se que em Julia matrizes são sempre do mesmo tipo. Então ao criar uma matriz por concatenação, por exemplo por `[x y]` a linguagem irá converter todos os valores para um tipo comum entre as duas colunas, se possível. Nesse caso seria para `Float64`. Já em Data Frames, as colunas podem ter tipos diferentes.
"""
...
Ler e escrever CSVs pode ser delicado. Isso porque diferentes línguas têm diferentes convenções. Por exemplo, em português a vírgula tem um significado especial. Ela representa o ponto do número onde a informação passa representar valores menores que a unidade. Em inglês esse símbolo é o ponto. Nesse sentido é bem comum arquivos CSV gerados por planilhas em português usarem outros símbolos para separarem os elementos de cada coluna, como o ponto-e-vírgula. Há ainda outros exemplos onde essas diferenças podem ser importantes, como em datas.

A função `read` da `CSV.jl` possui várias opções para costumizar a leitura, em particular para lidar com essas diferenças associadas a linguagem. Leia com calma a sua [documentação](https://csv.juliadata.org/stable/reading.html#CSV.read).
"""
...
md"""
Essa aula vai tentar explicar melhor o significado dos valores da tabela.
"""

# ╔═╡ 469d809f-424f-4595-ad43-a5b2cc055304
md"""
# Algumas palavras (na verdade código) sobre regressão
"""
...
function linear_regression(x, y)      # a direct computation from the data
    ...
    σᵉ = sum((mᵉ * x .+ bᵉ .- y) .^ 2) / (n - 2) # squared noise estimate
    bᵉ, mᵉ, σᵉ
end
...
Lembre, num modelo há três tipos diferentes de variáveis. Os _parâmetros_ do modelo, b, m e σ no exemplo acima, que estamos consideramos desconhecidos. Há também as variáveis _preditivas_, consideradas conhecidas (sem ruído), e as variáveis de _resposta_ que são ruidosas.
"""
...
-7.21066 / 23.029, 0.44019 / 0.520996
...
md"""
Em uma aula de estatística você deve ter encontrado (ou vai encontrar) uma variável aleatória com distribuição t (e parâmetro k). Ela representa a razão entre uma normal padrão e uma distribuição Χ com parâmetro k e corrigida por um fator $\sqrt{k}$. Vamos simular isso a partir de `randn` e da definição da Χ. Para os dados nos experimentos com muitas amostras (mais comum nos dias de hoje), a distribuição normal está próxima da t (já o que o k está associado com o número de amostras, que é alto), ao ponto não se usar mais a t. De qualquer forma, usando uma t ou uma normal, usamos uma distribuição porque estamos conscientes que o valor real de σ é desconhecido. Ele foi obtido por estimativa.
"""
...
rand_t(k) = sqrt(k) * randn() / rand(Chi(k)) #norm(randn(k))
...
md"""
Em estatística nós perguntamos se um coeficiente deve ser considerado 0 (que diz que os dados não tem intercepto ou não dependem de x) ou se os coeficientes estimados são significativos com alguma probabilidade. A conluna `Pr(>|t|)` nos dá a probabilidade de aceitarmos a hipótese nula, ou seja que o respectivo coeficiente deve ser simplesmente 0.

Em um teste estatístico correto, você deve decidir em qual nível aceita (ou rejeita) a hipótese. Por exemplo com 0.01, 0.05 ou 0.1 podem ser valores razoáveis. Se o teste der uma probabilidade menor você deve aceitar que os coeficientes são significativos. Você não deve decidir o volor, por exemplo, 0.01 depois de ver os resultados.
"""
...
manifest_format = "2.0"
project_hash = "7c07529178538e044d6c642fa4d96a335454cfbc"
