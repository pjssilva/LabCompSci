...
md"""
### Pré-pesamento computacional (soluções tradicionais de EDOs por meios analíticos)
"""
...
Vamos usar a notação matemática tradicional ``y'`` para a derivadad de $y$ com relação ao tempo.
"""
...
Comentário:  O ponto de equilíbrido é obtido (caso $b \ne 0$)  resolvendo `y' = 0` para `y`. Nesse caso, isso resulta em $y = a / b$. Ele também pode ser obtido tomando $t \rightarrow \infty$ na solução. Isso vai  "eliminar" o termo exponencial. Lembre-se da última aula, o equilíbrio é um atratator para as trajetórias quando o tempo passa.
...
md"""
#### ``y'=`` linear em y + "termo forçante" em t:
...
md"""#### 1) Prerequisitos: Física climática
...
S = 1368; # Insolação solar [W/m^2]  (energia por unidade do tempo por unidade área)
...
md"""
Na matemática, nós nos contentamos com a equação diferencial. Mas no mundo físico essas equações dependem de parâmetros que precisam ser identificados.
...
Como os raios solares incidentes são aproximadamente paralelos a esse (longa) distância do Sol, a seção tranversal da área da Terra que que intercepta esses raios é um disco com área $\pi R^{2}$, em que $R$ é o raio do planeta. Como todos os outros termos que vamos considerar "agem" sobre toda a superfície esférica terrestre, que $4\pi R^{2}$, a absorção de radiação solar por *unidade de área de superfície* (considerando a média sobre todo o globo) é reduzida por um fator de 4.
...
Para isso consideramos que o mundo preindustrial estava em balanço, e dessa maneira a temperatura de equilíbrio é a temperatura média daquela época. Os restorno linearizados assumem então a forma
...
begin
    ...
    CO2_historical_data = CSV.read(
        download(CO2_historical_data_url), DataFrame; header=55, datarow=58
    )
...
begin
    ...
    plot!(
        parse.(Float64, T_df[:, 1]),
        parse.(Float64, T_df[:, 2]) .+ 14.15;
        color=:black,
...
md"""
No cenário de baixas emissões o acréscimo na temperatura fica baixo de  $ΔT = 2$ °C até 2100, enquanto no cenário de altas eissões chega a 3.5 °C acima dos níveis preindustriais.
"""

# ╔═╡ fef0a76b-98dc-42a0-975b-0fba8a32e5cc
md"
Apesar do efeito estufa assciado a emissão de CO₂ se o fator forçante dominante por detrás do aquecimento histórico e projetado para o futuro, a modelagem climática modernaconsiderando uma lista bastante exaustiva de fatores forçantes (aerosois, outros gases de efeito estufa, ozônio, mudanças no uso do solo, etc.). O vídeo abaixo apresenta uma lista desses fatores forçanes considerados em simulações climáticas do período histórico feitas com modelos do estado da arte."
...
git-tree-sha1 = "d6553d26ecdda5ed362743e1e095a8ada11b0372"
...
git-tree-sha1 = "26bdb90844e9fcdc6d86f7a16218ae7801f492cb"
...
git-tree-sha1 = "e00afbec5b3f3793c605d2f1e19c74332f006408"
...
git-tree-sha1 = "46b37a324b0b66a5150c429aba080fcfc80cbc66"
