...
md"""
Nesse caderno vamos ver que o modelo mais simples possível para o clima, um conjunto de três EDOs acopladas conhecido como **Equações de Lorenz** podem apresentar um comportamento imprevisível chamado **caos (determinístico)**. Vamos chegar lá começando de dois modelos ainda mais simples com dinâmicas uni e bidimensionais.
"""
...
md"""
Lembre-se, tipicamente iríamos evitar o método de Euler para calcular as trajetórias, mas ele é bom o suficiente para entender o comportamento _qualitativo_ da equação. E vamos usá-la justamente porque o conhecemos bem.
"""
...
md"""
Vemos que no **valor crítico** $\mu_c = 0$ ocorre uma mudança **qualitativa no comportamento** do sistema. Para $\mu_x < 0$ exitem dois pontos fixos. Já para $\mu > 0$ não há nenhum. Esse tipo de mudança qualitativa de comportamento é denominado uma **bifurcação**. Nesse caso particular, os dois pontos fixos colidem em uma bifurcação **sela-nó** (dois equilíbrios "colidem" e se anulam). 
"""
...
md"""
Vemos que há uma faixa de valores $\mu$ nos quais **coexistem três pontos fixos**, dois estáveis e um instável. Como há dois pontos fixos estáveis para onde convergir dizemos que esse sistema é **biestável**.
"""
...
md"""
Agora imagine que vamos modificar o parâmeto $\mu$ vagarosamente. Ao mudarmos o parâmetro um pouco, o sistema não estará mais em um ponto fixo, já que a posição do ponto fixo muda ao alterarmos o parâmetro. Entrentato, o sistema irá **relaxar**: seguirá a dinâmica associada ao novo valor de $\mu$ e convergirá rapidamente para o novo ponto fixo que está próximo.
...
b = $(@bind b Slider(0.0:0.1:5.0, show_value=true, default=1.0))
"""
...
md"""
As [equações de Lorenz](https://en.wikipedia.org/wiki/Lorenz_system) são um modelo (muito simplificado) de convecção em uma camada de fluido representando a atmosfera. Ela foi estudada inicialmente em um artigo de Edward Lorenz, publicado em 1963, sobre um trabalho feito no MIT.  Elas representam um trabalho pioneiro da investigação numérica de **comportamento caótico**.
