...
# ╔═╡ 733289d7-ab35-4a80-869e-61d43e2bced6
md"Tradução livre de [our\_first\_climate\_model.jl](https://github.com/mitmath/18S191/blob/Spring21/notebooks/week12/our_first_climate_model.jl)."

# ╔═╡ f17bad51-15c6-4018-a974-83a3bcc0d5fc
TableOfContents(; title="📚 Índice", indent=true, depth=4, aside=true)

# ╔═╡ 92883ed9-5572-41fd-96c7-190279f90804
md"""
Essa aula é baseada na versão do outono de 2020 de "Introdução à Modelagem Climática" por Henri Drake. [Veja o vídeo](https://computationalthinking.mit.edu/Fall20/lecture20/),
[Caderno original do prof. Drake](https://github.com/mitmath/18S191/blob/Fall20/lecture_notebooks/week11/01_energy_balance_model.jl) em que ele descreve o Modelo de Balanço Energético.
"""

# ╔═╡ 340fc7c4-128f-4476-b445-65005ffa9f5a
md"""
### Antes de ir para o computador (soluções tradicionais de EDOs por meios analíticos)
"""
...
``y = y(t)`` é uma função do tempo $t$ 

No instante inicial $t = 0$, temos o valor (conhecido) $y(0) = y_0$.

Vamos usar a notação matemática tradicional ``y'`` para a derivada de $y$ com relação ao tempo.
"""
...
Solução: ``y(t) =(- \frac{a}{b})(e^{-bt}-1) + y_0 e^{-bt}``

Comentário:  O ponto de equilíbrio é obtido (caso $b \ne 0$)  resolvendo `y' = 0` para `y`. Nesse caso, isso resulta em $y = a / b$. Ele também pode ser obtido tomando $t \rightarrow \infty$ na solução. Isso vai  "eliminar" o termo exponencial. Lembre-se da última aula, o equilíbrio é um atrator para as trajetórias quando o tempo passa.
...
md"""
#### ``y'=`` linear em y + "termo forçante" em t (equação não é mais homogênea):


``y' = a - by + f(t)``

Solução: ``y(t) = e^{-bt} \left( y_0 + \int_0^t  e^{bu}(a+f(u)) du \right)``
...
md"""
#### Revisão: Resolvendo uma EDO ($y' = f(y)$) em Julia:

0. Defina `f(y, p, t)`.
1. `ODEProblem(f, y0, time_span, parameters)`.
2. Use `solve`.
3. Calcule valores da solução, faça gráficos com `plot`, etc.
"""
...
f(y, (a, b, forcing), t) = a - b * y + forcing(t)
...
begin
    # You need to configure the arrow type directly in the GR backend
    GR.setarrowsize(0.5)
    ...
    plot!(xs, ys; arrow=true, c=:lightgray, alpha=0.3)
    ylabel!("y")
    annotate!(-0.5, y₀, text("y₀"; color=:red))
    title!("Solution to y'(t) = a - by + forcing(c)(t)")
end

# ╔═╡ aba7fc93-c0ac-4c9e-b975-18145f87707f
md"""#### 1) Pré-requisitos: Física climática

O modelo climático conceitual mais simples é:

$\begin{align}
\text{\color{brown}{Mudança na quant. de calor}} = & + \text{\color{orange}{Radiação solar absorvida (energia dos raios solares)}}\\
& - \text{\color{blue}{Radiação térmica perdida}} \\
&\hspace{2.15cm}\text{\color{blue}{(resfriamento de corpo negro para o espaço)}}
\newline
& + \text{\color{grey}{efeito estufa de causa humana}} \\
&\hspace{5.75cm} \text{\color{grey}{(radiação de saída retida)}}
\end{align}$

Cada uma desses valores devem ser interpretados como médias para todo o globo terrestre (ou seja são "zero-dimensionais").
"""
...
md"""
### 1.1 🌞 de entrada: radiação solar absorvida (um exemplo de $\mathrm{temp}' = \text{constante}$)
"""

# ╔═╡ 087f47b2-8283-4205-88f2-4d5883a340c2
md"""
Na distância orbital da Terra ao Sol, a potência dos raios solares que interceptam a superfície do planeta é de:
"""

# ╔═╡ 9c89c4e9-65ee-4424-bd74-17168b211797
S = 1368; # Insolação solar [W/m^2]  (energia por unidade área)

# ╔═╡ ca5b41d2-b511-486a-91fe-cceb8f7282c3
md"Uma pequena fração"

# ╔═╡ 13984c61-4c34-40db-9043-fcff2721522e
α = 0.3; # albedo, ou refletividade planetária [adimensional]

# ╔═╡ d74936e9-b760-4add-b3e7-46d544064c16
md"""
Na matemática, nós nos partimos da equação diferencial, imaginando que ela é conhecida. Mas no mundo físico essas equações dependem de parâmetros que precisam ser identificados.

Na nossa construção do modelo de aquecimento, vamos usar as quantidades a seguir:

- Início da Revolução Industrial: 1850.
- Temperatura média em 1850: $14.0 °C.
- Insolação solar $S = 1368$ W/m^2: energia incidente do Sol.
- Albedo ou coeficiente de reflexão (do planeta): $\alpha = 0.3$.
- Capacidade de calor da atmosfera e da camada superior dos oceanos: $C = 51$ W yr /m²/°C. 

Usando esses dados podemos chegar ao seguinte modelo simples de ganho de temperatura:

`` C\  \mathrm{temp}'(t) = S(1 - α)/4 = `` $(S*(1-α)/4)

Vamos entender um pouco de onde ele vem.
"""

# ╔═╡ 868e19f4-d71e-4222-9bdd-470387991c67
md"""
dessa radiação solar é refletida de volta ao espaço (por superfícies refletivas como nuvens brancas, neve e gelo). A fração restante $(1-\alpha)$ é absorvida.

Como os raios solares incidentes são aproximadamente paralelos a essa (longa) distância do Sol, a seção transversal da área da Terra que intercepta esses raios é um disco com área $\pi R^{2}$, em que $R$ é o raio do planeta. Como todos os outros termos que vamos considerar "agem" sobre toda a superfície esférica terrestre, que $4\pi R^{2}$, a absorção de radiação solar por *unidade de área de superfície* (considerando a média sobre todo o globo) é reduzida por um fator de 4.

![](https://www.open.edu/openlearn/ocw/pluginfile.php/101161/mod_oucontent/oucontent/890/639dcd57/ce3f1c3a/s250_3_002i.jpg)

Chegamos em

$\textcolor{orange}{\text{Energia (radiação) solar absorvida} \equiv \frac{S(1-\alpha)}{4}}.$
"""
...
md"""
Essa energia absorvida, no nosso modelo simples que, pelo momento, apenas considera a energia que entra, deve resultar em uma mudança na quantidade de calor (energia) do sistema. Esse valor é basicamente ``C \cdot \rm{temp}`` em que $\rm{temp}$ é a temperatura do sistema em Kelvin e $C$ é a capacidade de calor climática do sistema.

Lembrem que estamos interessados na temperatura da atmosfera, que tem uma baixa capacidade de calor, mas cuja temperatura é fortemente acoplada à temperatura da porção superior dos oceanos, que possui uma capacidade de calor muito maior.
"""

# ╔═╡ cc9a1e1c-762a-4a35-8480-b83a184beb65
C = 50.9; # capacidade de calor da atmosfera e da superfície dos oceanos [ W yr/m^2/°C]

# ╔═╡ 2086e386-194b-4863-a9ee-178002925d42
md"""
A *variação da quantidade de calor ao longo do tempo* é então dada simplesmente por  $\frac{d(C\ \rm{temp})}{dt}$. Como a capacidade de calor da água do mar é basicamente constante com a temperatura, podemos tirar $C$ como constante chegando a 

$\color{brown}{\text{mudança na quantidade de calor} =\; C \frac{d\ \rm{temp}}{dt}}$
"""

# ╔═╡ 644e13c5-8615-493d-a00f-8fc0b720cf0b
md"Podemos então igualar esses dois valores na equação diferencial final

$C\  \mathrm{temp}'(t) = S(1 - α)/4.$

Agora estamos prontos para resolver a equação numericamente (mas é claro que também sabemos a solução analítica).
"

# ╔═╡ 91329865-f593-4388-a5d0-01af4be6e01d
temp₀ = 14.0 # temperatura média pre-industrial [°C]
...
md"""
### 1.2 ♨ de saída : radiação térmica
"""

# ╔═╡ b186d026-d02c-4f06-8097-aa54b363f4fb
md"""
O termo da energia térmica que se perde $\mathcal{G}(T)$ (ou "resfriamento de corpo negro para o espaço") representa os efeitos combinados dos *retornos (_feedbacks_) negativos que diminuem o aquecimento*, como a *radiação de corpo negro* e os *retornos naturais positivos que amplificam o aquecimento*, como o *retorno de vapor d'água*.

Como a física é muito complicada para discutirmos aqui, vamos *linearizar* o modelo olhando apenas o primeiro termo da série de Taylor 

$\mathcal{G}(T) \approx \mathcal{G}(T_{0}) + \mathcal{G}'(T_{0})(T-T_{0}) = \mathcal{G}'(T_{0})T + (\mathcal{G}(T_{0}) - \mathcal{G}'(T_{0})(T_{0}))$

em torno da temperatura de equilíbrio pré-industrial de $T_0 = 14°C$.

Vamos simplificar a expressão definindo:

$\begin{align}
& A \equiv \mathcal{G}(T₀) - \mathcal{G}'(T₀)(T₀) & \\\\
& B \equiv -\mathcal{G}'(T₀) & \text{(O parâmetro de retorno climático)},
\end{align}$

que leva a

$\color{blue}{\text{Radiação térmica perdida} \equiv \mathcal{G}(T) \approx A - BT}.$
"""

# ╔═╡ edafca88-dd4a-4fe4-8ffc-bb50f7bd561d
md"""
O valor de $B$ que vamos usar é
"""

# ╔═╡ 9b97e41b-76ff-4cc2-90c1-446d59c4ece7
B = -1.3; # climate feedback parameter [W/m^2/°C],

# ╔═╡ 17af399b-724d-4e14-8158-5e7b1e02b92f
md"""
Ele vem de estimativas baseadas na compreensão dos diferentes retornos (_feedbacks_) que afetam o clima. Leia mais sobre isso [aqui](https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&ved=2ahUKEwikwbfrm9LsAhVjhuAKHQhZCm8QFjAEegQIAhAC&url=https%3A%2F%2Fclimateextremes.org.au%2Fwp-content%2Fuploads%2F2020%2F07%2FWCRP_ECS_Final_manuscript_2019RG000678R_FINAL_200720.pdf&usg=AOvVaw0hWIM3t4kJTovxoeobcRIN).

**Observação:** O sinal $B < 0$ modela que o retorno climático é **negativo** (tem efeito estabilizante). Efeitos de retorno positivo levam $B$ a ficae menos negativo. Isso reduz a eficiência com que a Terra é capaz de se resfriar ao irradiar energia térmica para o espaço. Isso aplificaria o aquecimento.

Já o valor de $A$ é definido a partir do equilíbrio préindustrial: antes da ação humana o orçamento energético da Terra estava perfeitamente equilibrado. Ou seja,

$\text{\color{orange}{Radiação solar absorvida}} = \text{\color{blue}{Radiação solar perdida}}$

ou

${\color{orange}{\frac{S(1-\alpha)}{4}}} = {\color{blue}{A - BT_{0}}}$

Reorganizando essa equação conseguimos calcular $A$:
"""

# ╔═╡ 314a0b76-a1d0-4337-bd33-efec373886a7
A = S*(1. - α)/4 + B*temp₀; # [W/m^2].

# ╔═╡ 719d4646-77f2-43fa-a431-3b1cb77a367a
...
p2 = ODEProblem(
	(temp, p, t) -> (1 / C) * (
		absorbed_solar_radiation - outgoing_thermal_radiation(temp)
	), start_temp, (0.0, 170))
...
begin
    ...
    hline!([temp₀, temp₀]; c=:white, ls=:dash)
    annotate!(
        80, temp₀, text("Preindustrial Temperature = $(temp₀)°C", :bottom; color=:white)
    )
...
md"""
### 1.3 Estufa 🏭: O efeito estufa causado pela humanidade]

Empiricamente, o feito estufa depende de forma logarítmica das concentrações de dióxido de carbono na atmosfera

$\color{grey}{\text{Efeito estufa antropogênico}\; = {\mbox {(coef\_forçante)}} \ln \left( \frac{[\text{CO}₂]}{[\text{CO}₂]_{\text{PI}}} \right),}$

Como isso vai variar do tempo depende do comportamento humano. A variação no tempo não está sendo modelada na equação acima.
"""
...
CO₂_PreIndust = 280.0; # preindustrial CO2 concentration [parts per million; ppm];

# ╔═╡ 437faadd-0301-403a-bcd7-18ce279589d0
greenhouse_effect(CO₂) = forcing_coef * log(CO₂ / CO₂_PreIndust)

# ╔═╡ ba559602-6b84-4e32-a548-98cefdeca05b
md"""
Agora, precisamos entender como vem sendo a evolução da concentração de gás carbônico na atmosfera partindo do nível 280, pré-industrial, até os dias de hoje. Para isso vamos tentar ajustar uma curva à medidas históricas.
"""

# ╔═╡ c6f8dcf6-950d-48ff-b040-55bdd347d74b
md"""
### Observações do Vulcão Mauna Loa![Vulcão Mauna Loa](https://i.pinimg.com/originals/df/1a/e7/df1ae72cfd5e6d0d535c0ec99e708f6f.jpg)
"""
...
begin
    ...
    CO2_historical_data = CSV.read(
        download(CO2_historical_data_url), DataFrame; header=62, skipto=65
    )
end
...
md"""
O dado de interesse está na quinta coluna, chamada CO₂.
"""

# ╔═╡ ef9d9512-cb40-4641-9d6a-5a36f5a4b33d
md"""
Ah não!  Tem dado faltante: -99.99.
"""
...
# ╔═╡ 4852d92c-186b-4783-bef5-c4ff4923598d
md"""
Não parece nada bom...

Podemos também considerar a equação diferencial completa e resolvê-la para tentar estimar a variação nas temperaturas.

``
\begin{gather}
{\color{brown}{C \frac{d\ temp}{dt}}}
= {\color{orange} \frac{S(1 - α)}{4}} - {\color{blue} (A - BT)} + 
{\color{gray} \mbox{(coef\_forçante)} \ln \left(  \frac{[\text{CO}₂]}{[\text{CO}₂]_{\text{PI}}} \right)}.
\end{gather}
``

Isso na linguagem do `DifferentialEquations.jl` é:
"""
...
p3 = ODEProblem(
    (temp, p, t) -> (1 / C) * (
		...
		outgoing_thermal_radiation(temp) + 
		greenhouse_effect(CO₂(t))
	),
    start_temp,
    (0.0, 170),
)

# ╔═╡ 71ffb3c9-4484-495e-ac59-739a4e96ee9f
solp3 = solve(p3);
...
md"""
Vamos usar esse ajuste para comparar com dados de temperaturas médias históricas.
"""
...
begin
    ...
    T_df = CSV.read(download(T_url), DataFrame;
        header=false,
        skipto=6,
        delim=" ",
        ignorerepeated=true,
        types=[Int, Float64, Float64]
    )
end;

# ╔═╡ 03576be2-0465-453d-a33e-41be454ccf9c
T_df
...
begin
    ...
    plot!(T_df[:, 1], T_df[:, 2] .+ 14.15;
        color=:black,
        label="NASA Observations",
        legend=:topleft
    )
end
...
begin
    begin
        plot(
            CO2_historical_data[validrowsmask, "      Date"],
...
md""" #####  Melhor e pior-caso: projeções para o aquecimento global futuro

"""

# ╔═╡ 13b003d2-1fd4-4a4a-960c-4a1d9b673dc6
md"""Vamos considerar dois futuros hipotéticos distintos:
1. Um mundo com **baixas-emissões** em que as emissões caem de modo a garantir concentrações de CO2 baixo de 500 ppm em 2100 (conhecido nos meio climático como  "RCP8.5") e
2. Um mundo com **altas-emissões** em que as emissões continuam crescendo até a concentração de CO2 subir a 1200 ppm ("RCP2.6").
"""
...
md"""
No cenário de baixas emissões o acréscimo na temperatura fica baixo de  $ΔT = 2$ °C até 2100, enquanto no cenário de altas emissões chega a 3.5 °C acima dos níveis pré-industriais.
"""

# ╔═╡ fef0a76b-98dc-42a0-975b-0fba8a32e5cc
md"
Apesar do efeito estufa associado a emissão de CO₂ ser o fator forçante dominante por trás do aquecimento histórico e projetado para o futuro, a modelagem climática moderna considera uma lista bastante exaustiva de fatores forçantes (aerossóis, outros gases de efeito estufa, ozônio, mudanças no uso do solo, etc.). O vídeo abaixo apresenta uma lista dos fatores forçantes considerados em simulações climáticas do período histórico feitas com modelos do estado da arte."
...
manifest_format = "2.0"
project_hash = "af84c0c8832e1c9d125fde26116babbff588374c"
...
git-tree-sha1 = "e17367f052035620d832499496080f792fa7ea47"
