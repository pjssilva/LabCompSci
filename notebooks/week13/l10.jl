...
md"_instalando pacotes úteis:_"

# ╔═╡ fe3304f8-2668-11eb-066d-fdacadce5a19
md"""
_Lembre-se de assistir as aulas sobre modelagem climática antes de tentar fazer essa lista. Algumas rotinas que vimos nessas aulas serão adiconanadas na célula abaixo. É uma boa ideia dar uma lida nelas antes de começar._
"""
...
ou seja, até onde sabemos esse valor obedece a uma distribuição normal com média  $\overline{B} = -1.3$ W/m²/K e desvio padrão  $\sigma = 0.4$ W/m²/K. Esses valores não são muito intuitivos. Assim, o nosso objetivo é interpretá-los como números mais relevantes para definição de políticas de mitigação.
...
md"""
👉 Em qual ano espera-se atingir a duplicação da concetração de CO₂ sob o cenário RCP8.5?
"""
...
Um valor de ``B`` próximo a zero significa que um aumento nas concentrações de CO₂ terá um impacto maior no aquecimento global, e que mais ações são necessárias para ficar abaixo de uma temperatura máxima. Ao responder essa questão relacionada à política, precisamos levar em consideração a incerteza associada com nossas estimativas para ``B``. Neste exercício, faremos isso usando uma simulação de Monte Carlo: geramos uma amostra de valores para ``B`` e usamos esses valores em nossa análise.
"""
...
md"""
👉 Gere uma amostra da distrbuição de probabilidade da ECS baseada na amostra da distribuição de $B$ obtida acima. Grafique um histograma.
"""

# ╔═╡ 3d72ab3a-2689-11eb-360d-9b3d829b78a9
# Solução ex. 1.3.a (não apague esse comentário)

ECS_samples = [0.0]  # Corrija com seu código
...
begin
	mean_ECS = 0.0 # Corrija com seu código
	ECS_mean_B = 0.0 # Corrigja com seu código
	prob_bigger = 0.0 # Corrija com seu código
...
- a função `CO2`, que mapeia um ano `t` na concetração de CO₂ daquele ano. Por exemplo, podemos usar a função `t -> 280` para simular um modelo com concetrações de gás carbônico fixas em 280 ppm.
...
Você pode inicializar uma intância de um `EBM` da seguinte forma:
"""
...
- Reinicie o modelo colocando transformando  `ebm.t` e `ebm.T` em vetores de um único elemento. _Qual o valor adequado?_
...
md"""
Se você quiser, deixe a sua visualização mais informativa. Como no cadenor da aula, você poderia deixar um rastro atrás do ponto negro, ou você poderia apresentar os ramos estáveis e instáveis. Isso fica com você (e é opcional)!
"""
...
👉 Encontre a **menor concetração de CO₂** necessária para derreter a bola de neve. Isso deve ser feito de forma "automática" (ou seja, usando código). A precisão mínima deve ser de 0.01 em escala logarítmica.
"""
