...
2. Use `solve`.
3. `plot`, calcule valores da solução, etc.
"""
...
$\begin{align}
\text{\color{brown}{Mudança na quantidade de calor}} = & + \text{\color{orange}{Radiação solar absorvida (energia dos raios solares)}}\\
...
$\textcolor{orange}{\text{Energia solar absorvida} \equiv \frac{S(1-\alpha)}{4}}.$
"""
...
md"""
Essa energia absorvida, no nosso modelo simples que, pelo momento, apenas considera a energia que entra, deve resultar em uma mudança na quantidade de calor (energia) do sistema. Esse valor é basicamente ``C\ \rm{temp}`` em que $\rm{temp}$ é a temperatura do sistema em Kelvin e $C$ é a capacidade de calor climática do sistema.

Lembrem que estamos interessados na temperatura da atmosfera, que tem uma baixa capacidade de calor, mas que tem sua temperatura é fortemente acoplada à temperatura da porção superior dos oceanos, que possui uma capacidade de calor muito maior.
"""

# ╔═╡ cc9a1e1c-762a-4a35-8480-b83a184beb65
C = 51.0;     # capacidade de calor da atmosfera e da superfície dos oceanos [ W yr /m^2/°C]
...
Como a física é muito complicada para discutirmos aqui, vamos *linearizar* o modelo combinando os efeitos positivos e negativos. 

Para isso consideramos que o mundo preindustrial estava em equilíbrio, e dessa maneira a temperatura de equilíbrio é a temperatura média daquela época. Os retornos linearizados assumem então a forma

 
$\rm{temp}'(t) = B(\rm{temp}(0) - \rm{temp}(t)),$ 

para algum valor $B$.

O sinal de menos antes de $\rm{temp}(t)$ indica que O termo busca restaurar o equilíbrio.
"""

# ╔═╡ edafca88-dd4a-4fe4-8ffc-bb50f7bd561d
md"""
O valor que vamos usar é
"""
...
$\color{grey}{\text{Efeito estufa antropogênico}\; = {\mbox {(coef\_forçante)}} \ln \left( \frac{[\text{CO}₂]}{[\text{CO}₂]_{\text{PreIndust}}} \right),}$

Como isso vai depender do tempo depende do comportamento humano. A variação no tempo não está sendo modelada na equação acima.
"""
...
begin
    ...
    plot!(
        [CO₂_PreIndust],
        [greenhouse_effect(CO₂_PreIndust)];
        marker=:circle,
        ms=6,
        linecolor=:white,
        color=:blue,
...
p3 = ODEProblem(
    (temp, p, t) -> (1 / C) * (B * (temp₀ - temp) + greenhouse_effect(CO₂(t))),
    start_temp,
    (0.0, 170),
)
...
begin
    ...
    CO2_historical_data = CSV.read(
        download(CO2_historical_data_url), DataFrame; header=55, skipto=58
    )
...
p4 = ODEProblem(
    (temp, p, t) -> (1 / CC) * (BB * (temp₀ - temp) + greenhouse_effect(CO₂(t))),
    start_temp,
    (0.0, 170),
)
...
git-tree-sha1 = "da237a52d393f60cc04256b7bcd9e7ff328b20cd"
...
git-tree-sha1 = "3c99fc37cd2b7809b2dfcdc2eb89e87392811da8"
