...
Para essas notas de aulas queremos usar cadernos de Julia baseados na versão 1.6 da linguagem. Para os cadernos iremos utilizar o Pluto no lugar de Jupyter. Os cadernos Pluto têm algumas características interessantes como serem mais responsivos e não possuírem um estado que depende da ordem de execução das células. Nesse sentido são ideias para esse tipo de curso, já que nossas aulas serão sempre baseadas na execução de código "vivo".
...
using Pkg
Pkg.add(["Pluto", "Plots", "LaTeXStrings", "PyPlot", "Polynomials"])
```

Esses comandos irão instalar o [Pluto](https://github.com/fonsp/Pluto.jl), que é o sistema de cadernos. Essa operação, aparentemente simples, vai baixar vários pacotes e demora um pouco. Por isso sugiro deixar a Julia trabalhando e ir tomar um café. Note também que instalar Julia + Pluto irá consumir cerca de 6Gb de espaço em disco, caso você não tenha o Python + Matplotlib instaladso usando `conda` (ou Anaconda) disponível no seu `PATH`. Caso já tenha o Python + Matplotlib instalados, o uso de disco deve ser em torno de 1-2Gb. 
...
Isso vai iniciar o Pluto no seu diretório padrão do sistema operacional e o browser abrirá uma nova aba para você selecionar um caderno e trabalhar com ele. Em seguida, vale à pena você seguir a leitura e executar os comandos abaixo para testar se os pacotes foram instalados corretamente. Para notar alguma diferença mude a função e rode o código abaixo mais uma vez colocando o cursor lá e apertando shift-enter no teclado. Se deu tudo certo aparecerá o gráfico da sua nova função. Ah, e não esqueça de atualizar também o título da figura para a nova função. Tenha um pouco de paciência, a primeira vez que você tenta gerar um gráfico é demorado. Os próximos gráficos devem aparecer mais rapidamente.
"""
