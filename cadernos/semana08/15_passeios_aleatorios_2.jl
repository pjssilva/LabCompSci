...
\sum_{i = 1}^6 i = (1 + 2 + 3) + (4 + 5 + 6).
```
As duas primeira somas podem ser feitas em paralelo e no final há apenas uma soma para fazer.
...
Para isso vamos unir os dois conceitos. Vamos primeiro criar uma função calcular a média de uma trajetória aleatória completa e depois reduzir essas médias por somas e obter a média das médias.
"""
...
md"""
Note que para matrizes esparsas a representação visual inicia com números, mas depois de um tempo ela passa a enfatizar apenas a esparsidade, destacando as entradas não nulas. Isso revela a **estrutura de esparsidade** da matriz: as posições onde encontramos números não nulos. Essas posições são apresentadas com pontos (e os zeros ficam como espaços em branco).
"""

# ╔═╡ f4c9b02b-738b-4de1-9e9d-05b1616bee0b
md"""
O padrão que emerge é bem interessante e é parente próximo de uma outra figura que vocês já devem ter visto: o triângulo de Sierpinski.
"""
