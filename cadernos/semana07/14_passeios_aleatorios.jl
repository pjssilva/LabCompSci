...
md"""
# Motivação: Dinâmicas de esferas rígidas
"""

# ╔═╡ 66a2f510-9232-11eb-3be9-131febc0039f
md"""
Brown observou o **movimento browniano** em 1827: grandes partículas como poeira ou pollen se movimentavam na água de maneira aparentemente aleatória. Einstein explicou isso em 1905 como o resultado dos repetidos choques entre as moléculas de água.

Nós podemos visualizar esse fenômeno com uma simulação de esferas rígidas chocando-se uns contra as outras. Apesar da dinâmica não ser aleatória -- já que cada esfera segue as leis de Newton -- ao olharmos cada uma delas isoladamente o movimento de fato "parece" aleatório.
"""
...
Vamos visualizar uma dessas simulações ocorrendo no plano.
"""
...
um disco (já que estamos no plano) rígido.
...
- Mesmo que seja _possível_ levantar todos os detalhes, o uso de um modelo simplificado que use aleatoridade pode ajudar na compreensão do processo.
...
Vamos, mais uma vez, usar o pacote `BenchmarkTools.jl` que disponibiliza ferramentas simples para fazer essas medidas de tempo e colecionar estatísticas. Um exemplo é a macro `@btime` que ajuda a estimar o tempo real de execução. Para evitar o problema de performance associado a variáveis globais, todos os trechos que serão avaliados serão encapsulados em funções.
"""
...
begin
	...
	for i in 1:10
		plot!(walk1D(256), leg=false, size=(500, 300), lw=2, alpha=0.5)
	end
	
	plot!()
end

# ╔═╡ 2cd61bad-6c07-498d-84c3-cebd95a04bad
md"Já que tivemos tanta preocupação com o tempo de gerar cada um dos números aleatórios, será que de fato o código acima é o mais eficiente possível. Vamos medir o seu tempo de execução."

# ╔═╡ 6d633c09-e35a-4255-aec0-12c11cc384d3
@btime walk1D(8096)

# ╔═╡ ac034b63-48a9-4406-a730-2ed70061bd15
md"Um dos pontos no código que talvez leve a um pouco mais de ganho no tempo é evitar crescer o vetor que guarda a simulação a *cada* iteração. Isso deve exigir realoações e cópias. Para isso vamos usar fato que já sabemos o tamanho de vetor no início da função e pre-alocar o vetor inteiro para resposta no início."

# ╔═╡ d442ca19-9031-474a-baa1-44feb068fdff
function altwalk1D(N)
	x = 0
...
# ╔═╡ 1aa0b774-b1fa-4921-8e6d-66a2e0fd33f0
@btime altwalk1D(8096)
...
# First we create an abstract type to allow for general coding when possible.
...
Em princípio, nós podemos usar o método da última aula para modelar esse conceito. Por outro lado isso pode trazer dificuldades, pois estamos interessados em todos os passos intermediários.
...
md"""
Suponha que geramos os passos $X_i$, ou seja
"""
...
md"""
Note que há o mesmo problema das imagens: o que fazer com as bordas? Precisamos especificar **condições de contorno**. No caso acima colocamos zeros na fronteira. Isso corresponde a considerar que qualquer partícula que chega na fronteira em um certo instante do tempo irá escapar do sistema. _Em algumas situações químicas isso está relacionado com um químico que desaparece._ Obs: eu não tenho ideia do que essa frase quer dizer, fica para os nossos colegas químicos esclarecer. 
"""
...
manifest_format = "2.0"
project_hash = "e36c4d5093d1df3b86fd785cd844509c47dcfd84"
