...
# ╔═╡ 56648a47-1d8d-4597-9bd3-85d88a05213e
md"Tradução livre de [dynamicprogramming.jl](https://raw.githubusercontent.com/mitmath/18S191/Spring21/notebooks/week4/dynamicprograms.jl)."

# ╔═╡ a84fdba4-80db-11eb-13dc-3f440653b2b9
md"""
## Introdução à otimização (programação) dinâmica
"""

# ╔═╡ 938107f0-80ee-11eb-18cf-775802c43c2f
md"""
Uma pequena nota histórica. De onde vem o termo "programação" em nomes como programação dinâmica e programação linear? Esse termo vem de antes da era do computador digital, quando programação significava algo como "definir a ordem no tempo". Ele ainda é usado naturalmente em situações que não envolvem computadores tão diretamente, como em "programação de produção de uma fábrica". Nesse contexto o objetivo da "programação" era otimizar a tarefa, executá-la da forma mais eficiente possível. Assim, "programação linear" pode ser ententida como "executar da forma mais eficiente possível um problema que pode ser descrito por funções lineares (afins)". Ou seja, nesse contexto "programção == otimização". O mesmo vale para o assunto da aula de hoje, otimização (programação) dinâmica ou otimização (programação) convexa.

Mas o que é otimização dinâmica. Vamos introduzir a sua ideia principal através de um exemplo.
"""

# ╔═╡ eb043a90-8102-11eb-3b78-d590a23c83f4
md"""
### O problema da descida
"""

# ╔═╡ 5994117c-8102-11eb-1b05-671b7cf87a7e
md"""
Vamos considerar o seguinte problema: você começa no topo de uma coluna qualquer de uma matriz de pesos aleatória. Seu objetivo é descer do topo à base pagando os pesos das céluas que atravessa. Os únicos movimentos possíveis são "para baixo e para esquerda", "para baixo" e "para baixo e para direita".

O desafio é chegar na base pagando o **mínimo possível**. Como resolver esse problema? Como resolvê-lo de forma eficiente?

Vamos começar entendendo um pouco melhor o que queremos. Começamos por gerar um matriz aletória m × n.
"""
...
m = $(@bind m Slider(2:12, show_value = true, default=8))
"""
...
md"""
## Fixando um nó no meio do caminho
"""

# ╔═╡ 28f18aa2-8104-11eb-0c01-dbd14c760ecf
md"""
Vamos fixar um nó $(i, j)$ e focar em todos os caminhos que passam por ele.
"""

# ╔═╡ 37ebfa3e-80e5-11eb-166c-4ff3471ab12d
md"""
i= $(@bind fixi Scrubbable(1:m))
j= $(@bind fixj Scrubbable(1:n))
"""

# ╔═╡ 4d81a6f4-8104-11eb-1f06-5bb7a56c8406
md"""
Mesmo fazendo isso ainda temos muitos caminhos, além disso ainda não está muito claro o que fazer. Porém ao fixar um ponto, qualquer que seja uma observação vem à tona:
"""

# ╔═╡ 808d9c9b-79ef-421a-a794-1308eac43b7c
md"""
Uma das dificuldades fundamentais do problema é que não é fácil aproveitar esse fato. Ao final das contas, ao fixar um nó acabamos recalculando todos os possíveis caminhos que passam por ele e isso é feito repetidas vezes.

Será que podemos evitar esse recálculo?
...
1. Como sugerido, atualizar o custo de passar por um nó do seu valor individual para o valor total do melhor caminho que parte dele.
"""
...
# ╔═╡ 0d778662-7815-44e7-ac04-730375ca14fe
function compute_dyn_path(M)
    m, n = size(M)
    ...
    # Find the best place to start in the first row
    best_col = argmin([dyn_M[1, i].best_cost for i = 1:n])
    dyn_path = zeros(Int, m)
    dyn_path[1] = best_col
...
# ╔═╡ 56575e5d-5a11-4ac9-a88f-5370edfc4b42
dyn_path = compute_dyn_path(M)

# ╔═╡ c94df118-61bb-4a0b-9cd9-66972499bf5c
md"""
### Gerando todos os possíveis caminhos.

Vamos gerar todos os possíveis caminhos definindo um iterator que pecorre todas as possibilidades. Isso em Julia é feito pela [interface de iteração](https://docs.julialang.org/en/v1/manual/interfaces/).

Em Julia, quando a linguagem encontra o código
...
```
ela traduz isso imeditamente para
```julia
next = iterate(iter)
while next !== nothing
    (item, state) = next
...
```
Note que além do valor calculado, há um estado (`state`) separado. Isso pode ajudar em alguns casos.
...
begin
    ...
    function Base.iterate(p::Paths, state)
        # End when each row has an n
        if state ≠ fill(p.n, p.m)
            newstate = next_path(state, p.n)
            return newstate, newstate
        end
        return nothing
    end

    function next_path(path, n)
        k = length(path)
    ...
    # Compute a vector with all possible paths
    function allpaths(m, n)
        # This is a vector of paths (each path is a vector of Int)
        v = Vector{Int}[]

        # Iterate among all possible paths and save the values
        for p ∈ Paths(m, n)
            push!(v, copy(p))
        end
...
begin
    ...
    md"Há um total de $numpaths caminhos possíveis."
end
...
md"""
Novo objetivo é somar os números ao longo de um caminho e encontrar o de menor valor.
O vencedor é o de número $winnernum.
"""

# ╔═╡ cccc68b2-334e-4558-9a63-601b15b10ac2
md"""
**Custo obtido com otimização dinâmica** = $(sum([M[i, dyn_path[i]] for i=1:m]))

**Melhor custo entre todos os caminhos** = $winnertotal
"""

# ╔═╡ 5dd22d0e-80d6-11eb-0541-d77668309f6c
md"""
Caminho $( @bind whichpath Slider(1:numpaths, show_value=true) )
"""
...
begin
    ...
    md"Número de caminhos passando por ($fixi, $fixj) = $number_of_fixedpaths"
end

# ╔═╡ ee2d787c-80e5-11eb-1930-0fcbe253643f
md"Caminho $( @bind whichfixedpath Slider(1:number_of_fixedpaths, show_value=true) )"
...
md"""
### Um pequeno benchmark
"""

# ╔═╡ 7b9ac8e7-a58e-43d7-ac38-10c9b522eeb3
...
# ╔═╡ 205be3fe-27b0-48e9-9ed4-757b702aeb7a
hint(text) = Markdown.MD(Markdown.Admonition("hint", "Ideia", [text]))

# ╔═╡ d015335b-2bed-43eb-9e77-352e6731700a
hint(
    md"Partindo de um nó fixo, o único caminho que realmente importa é o melhor caminho a partir daquele ponto. E o custo real, efetivo, de passar por aquele nó é a soma total desse caminho ótimo.",
)
