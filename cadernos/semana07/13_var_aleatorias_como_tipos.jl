...
$\mathbb{P}(X = x)$. Isso no caso discreto. Já para variáveis aleatórias não é possível definir a probabilidade de valerem um valor específico. O que medimos é a probabilidade do valor amostrado pertencer a "intervalos".
...
Algumas linguagens permitem falar naturalmente de várias funções associadas a uma distribuição/variável aleatória, mas não é possível falar da distribuição/variável aleatória em sim.
...
Nós queremos definir tipos que recebam os parâmetros da distribuição de probabilidade de uma variável aleatória antes de definir exatamente como amostrá-la. Esse é um bom exemplo em que alguma **abstração** com antecedência traz ganhos interessantes.
