...
Isso permite alterar rapidamente código baseado em `rand` para usar uma nova distribuição. Mas para isso é preciso estender a função original para entender que está recebendo uma nova distribuição. 
...
md"""
Isso sugere que mesmo que toda informação de nossa distribuição Bernoulli possa ser guarda em uma simples variável Float64, é melhor criar um tipo específico para ela. Assim fica fácil estender funções com novos métodos que serão adaptados para esse novo tipo. 

Se nós guardamos a informação de nossa distribuição Bernoulli em um `Float64`, não há muito como fazer essa extensão. Além disso guardar esse informação em um `Float64` comum não deixa claro que esse número na verdade representa outro conceito: uma distribuição Bernoulli. Esse é mais um motivo para cria Sr um tipo específico.
"""
...
md"""
Como disse acima, queremos amostrar a distribuição usando `rand`. Podemos também querer calcular a sua média e desvio padrão. Para isso vamos estender as respectivas funções com novos métodos especializados. Vocês farão mais disso na lista.
"""
...
md"""
O objeto `B` representação uma distribuição Bernoulli com probabilidade $p$. Ela pode ser usada sempre que precisarmos de uma distribuição desse tipo. Você agora pode tentar criar o seu próprio mini pacote com sua distribuição simples. Assim não vai precisar fazer copia-e-cola quando precisar reutilizá-la. 
